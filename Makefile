...
SVTB1= ../tb/top.sv
INC = +incdir+../tb +incdir+../test +incdir+../wr_agt_top 
SVTB2 = ../test/gray_pkg.sv
...
help:
	...
	@echo "! run_test	=>  clean, compile & run the simulation for gray_test in batch mode.		!" 
	@echo "! view_wave1 =>  To view the waveform of gray_test	    						!" 
...
run_test_Questa: sv_cmp
	vsim -cvgperinstance $(VSIMOPT) $(VSIMCOV) $(VSIMBATCH1)  -wlf wave_file1.wlf -l test1.log  -sv_seed random  work.top +UVM_TESTNAME=gray_test
...
report_Questa:
	vcover merge mem_cov mem_cov
	vcover report -cvg -details -nocompactcrossbins -codeAll -assert -directive -html mem_cov

regress_Questa: clean_Questa run_test_Questa report_Questa cov_Questa
...
run_test_VCS:	clean  sv_cmp_VCS
	./simv -a vcs.log +fsdbfile+wave1.fsdb -cm_dir ./mem_cov +ntb_random_seed_automatic +UVM_TESTNAME=gray_test 
	urg -dir mem_cov.vdb -format both -report urgReport1
	
view_wave1_VCS: 
	verdi -ssf wave1.fsdb	
	
report_VCS:
	urg -dir mem_cov.vdb -dbname merged_dir/merged_test -format both -report urgReport
